...
function ProjectManageUI(props: IProsManage) {
  ...
  const statusState =
    complete &&
    isNaN((complete.length / props?.data?.fetchProject.task.length) * 100)
      ? 0
      : complete &&
        (complete.length / props?.data?.fetchProject.task.length) * 100;
              ...
              <S.ProjectListInfo>
                ...
                <li>
                  ...
                  <S.ProjectInfoValue>
                    ...
                    {props?.data?.fetchProject.method === "MEDIATE" &&
                      "조정 가능"}
                  </S.ProjectInfoValue>
                </li>
              </S.ProjectListInfo>
        ...
        <Wrapper paddingTop="0">
          <S.ProjectResponsiveWeb>
            <div>
              ...
              <div>
                {props.data?.fetchProject.projectMembers.length >= 1 ? (
                  props.data?.fetchProject.projectMembers?.map(
                    (el: any, index: number) => {
                      if (index === 0) {
                        return null;
                      }
            ...
            <div>
              <S.ProjectManageContentsTop>
                ...
                <div className="board_button">
                  <button
                    className="post"
                    id="add__button"
                    onClick={props.onClickonAdd}
                  >
                    글 올리기 +
                  </button>
                  <button
                    className="post board_delete"
                    id="add__button"
                    onClick={props.onClickAllDeleteBoard}
                  >
                    전체 삭제
                  </button>
                </div>
              ...
              <S.ProjectManaBoard>
                {props.data?.fetchProject.board.length === 0 ? (
                  <S.NonePost>게시물이 없습니다.</S.NonePost>
                ) : (
              ...
              <h3>
                ...
                <div className="to_do_status">
                  <S.ProjectInfoKey>
                    <S.TodoStatus>
                      <span>업무 진행률</span>{" "}
                      <span className="status__value">{`${Math.round(
                        statusState
                      )}`}</span>
                      %
                    </S.TodoStatus>
                  ...
                  <S.ProjectInfoValue>
                    <S.ProjectStatusBox>
                      <S.ProjectStatus status={statusState}></S.ProjectStatus>
                    </S.ProjectStatusBox>
                    ...
                    <button
                      id="add__button"
                      onClick={props.onClickAllDeleteTask}
                    >
                      전체 삭제
                    </button>
                  </S.ProjectInfoValue>
